    ...
    <div class="my-page">
      <div class="my-page_box">
        ...
        <div v-if="$store.state.clientOrWorker == 'worker'" class="worker_box">
          ...
          <div class="role-box">
            <p v-if="workerInfo.role_id" class="role">職種:{{ workerInfo.role.name }}</p>
            <p v-if="workerInfo.role_id == null" class="role">職種:未登録</p>
          </div>
          <div class="shop-box">
            <p class="shop-ttl">所属事業所情報</p>
            <p v-if="workerInfo.shop_id == null" class="shop-unregister">現在、登録情報はありません。</p>
            <p v-if="workerInfo.shop_id" class="shop-name">事業所名:{{ workerInfo.shop.name }}</p>
            <p v-if="workerInfo.shop_id" class="shop-type">分類:{{ workerInfo.shop.shop_type }}</p>
            <p v-if="workerInfo.shop_id" class="shop-email">メールアドレス:{{ workerInfo.shop.email }}</p>
            <p v-if="workerInfo.shop.email == undarfined" class="shop-email">メールアドレス:{{ workerInfo.shop.email }}</p>
            <p v-if="workerInfo.shop_id" class="shop-number">電話番号:{{ workerInfo.shop.number }}</p>
          </div>
        </div>
