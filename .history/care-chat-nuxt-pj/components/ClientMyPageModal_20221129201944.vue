        ...
        <div class="update-icon">
          <form>
            <input
              ...
              @change="checkIcon"
            />
            <button @click="updateIcon" class="update-name_btn">
              アイコンを登録する
            </button>
          </form>
  ...
  methods: {
    ...
    async updateIcon() {
      ...
      if (this.err.iconFile == false) {
        ...
        formData.append("client_id", this.$store.state.loginUserId);
        await this.$axios.post(
          "http://127.0.0.1:8000/api/v1/icon-change",
          formData,
          config
        );
      }
    ...
    async updateRelation() {
      ...
      const resultCheckRelation = await this.$axios.get(
        "http://127.0.0.1:8000/api/v1/relation-check",
        { params: checkRelation }
      );
      ...
      if (resultCheckRelation.data.data) {
        ...
        await this.$axios.put(
          "http://127.0.0.1:8000/api/v1/relation/" +
            resultCheckRelation.data.data.id,
          updateRelationInfo
        );
