  ...
  <div>
    ...
    <div class="Chat-area">
      <div class="comment-null"  v-if="comments.length == 0">
        <p>現在、チャットID:「{{ this.patient_name }}」に対するコメントはありません。</p>
      </div>
      <div class="v-for_box"  v-for="(comment,comment_index) in comments" :key="comment.id">
            ...
            <div class="client-balloon_comment-box">
              ...
              <p>{{comment_index}}:</p>
                <div class="index-check" >
                  <div class="put-delete" v-if="checkCommentIndex(comment.client_id)">
                      <img @click="showPut()" src="../img/put.png" alt="put" class="put_btn">
                      <img @click="showDelete()" src="../img/delete.png" alt="delete_btn" class="delete_btn">
                    
                  </div>
                </div>

              <modal name="modal-put">
                <p>{{ comment.content }}更新でモーダルウィンドウで表示されるコンテンツ</p>
              </modal>
              <modal name="modal-delete">
                <p>削除でモーダルウィンドウで表示されるコンテンツ</p>
              </modal>
    ...
    <div class="comment-form">
      <validation-observer ref="obs" v-slot="ObserverProps">
        ...
        <button @click="sendComment()" class="comment-form_btn"
          :disabled="ObserverProps.invalid || !ObserverProps.validated">コメントを送信</button>
      </validation-observer>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkCommentIndex(comment.client_id) {
      this.$store.state.clientOrWorker == 'client' && comment.client_id == this.$store.state.loginUserId
    },




    // コメント編集時、削除時のモーダル画面の出し入れ
    showPut() {
