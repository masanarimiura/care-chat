          ...
          <div class="client-balloon_img-box">
            ...
            <p class="client_name">
              <span v-for="relation in comment.client.relations" :key="relation.id">
                <span v-if="relation.patient_id == comment.patient.id">{{ relation.relation_type.name }}:</span>
              </span>
                <span v-if="comment.client.relations.find((v) => v.patient_id === comment.patient/id)">クライアント:</span>
...
export default {
  ...
  methods: {
    ...
    // client の コメントのrelationの検索からrelation_typeがない場合の表示
    searchRelation(commentClientRelations , commentPatientId){
      console.log(commentClientRelations)
      console.log(commentPatientId)
      const patientId = {
        patient_id : commentPatientId
      }
      const result = commentClientRelations.find((v) => v.patient_id === commentPatientId);
      console.log(result)
      // commentClientRelations.forEach(elm => {
      //   Object.keys(elm).forEach(key => {
      //     if(key === 'patient_id') {
      //       const relationPatientId = {
      //         patient_id : elm[key]
      //       }
