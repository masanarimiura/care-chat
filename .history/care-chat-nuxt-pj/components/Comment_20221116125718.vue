      ...
      <div class="oneArea">
        <div class="onebox">
          <div class="imgArea">
            ...
            <p class="worker_name">作業療法士:三浦雅成</p>
          </div>
          <div class="fukiArea">
            <div class="fukidasi">
              <p>
                こんにちは。◯◯◯◯様の最近の状況報告です。書くことで頭の中を整理できて、気持ちのリフレッシュにもなる日記。振り返ってみると、平凡に感じていた日常も意外な「気づき」に溢れているはず。日付を自分で書き込む「フリータイプ」や、毎日コツコツ書く「連用タイプ」、お子様の成長を記録する「育児日記」など種類豊富に取り揃えています。
              </p>
              <p class="datetime">2022.12.22 11:11</p>
            </div>
          </div>
        </div>
        <div class="onebox">
          ...
          <div class="fukiArea">
            <div class="fukidasi">
              ...
              <p class="datetime">2022.12.22 12:34</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="comment-form">
      <validation-observer ref="obs" v-slot="ObserverProps">
        <validation-provider v-slot="{ errors }" rules="required|max:1000">
          <p class="patient-make_box_textarea_ttl">コメントを書く</p>
          <textarea v-model="name" type="text" name="コメント" required placeholder="コメントの内容" cols="30" rows="10"></textarea>
...
export default {
  ...
  methods: {
    ...
    async make() {
      ...
      const getNewPatientId = await this.$axios.get("http://127.0.0.1:8000/api/v1/patient-search", { params: newPatientNamePass });
      ...
      // サンクスページで名前とパスワードを表示するために「名前」「パスワード」をクエリで送る。
      this.$router.push('thanks-make-patient');
    },
  },
  async created() {
...
.oneArea .onebox .imgArea .worker_name {
  position: absolute;
  top: 0px;
  color: rgb(80, 36, 0);
}
.oneArea .onebox .imgArea .client_name {
  position: absolute;
  top: 0px;
  left: 40px;
  color: rgb(80, 36, 0);
...
.oneArea .onebox .fukidasi .datetime {
  text-align: right;
  font-size: 16px;
  padding-top: 10px;
}
