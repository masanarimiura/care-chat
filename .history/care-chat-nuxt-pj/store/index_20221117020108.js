        ...
        if (user) {
          ...
          this.$axios.get("http://127.0.0.1:8000/api/v1/client", { params: searchUid })
            .then((clientResult) => {
              ...
              if (clientResult.data.data != undefined) {
                this.id = clientResult.data.data.id;
                this.name = clientResult.data.data.name;
                const userIdName = {
                  id: this.id,
                  name: this.name
                };
                ...
                commit('clientLogin')
              } else if ((clientResult.data.data == undefined)) {
                this.$axios
                  .get("http://127.0.0.1:8000/api/v1/worker", { params: searchUid })
                  .then((workerResult) => {
                    if (workerResult.data.data != undefined) {
                      this.id = workerResult.data.data.id;
                      this.name = workerResult.data.data.name;
                      const userIdName = {
                        id: this.id,
                        name: this.name
                      };
          ...
          // firebaseのログインチェックに引っかかった場合にvuexのストアもリセットする。
          commit('logout')
