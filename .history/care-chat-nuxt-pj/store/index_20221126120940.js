...
export const actions = {
  ...
  async onAuth({ commit }) {
    ...
    if (firebase.auth().currentUser.emailVerified == true) {
      // メールの確認が取れてたらvuex.storeの内容更新をしてログイン状態を作る
      firebase.auth().onAuthStateChanged(user => {
        if (user) {
          ...
          this.$axios
            .get("http://127.0.0.1:8000/api/v1/worker-check", { params: searchUid })
            .then((result) => {
              if (result.data.data != undefined) {
                this.id = result.data.data.id;
                this.name = result.data.data.name;
          ...
          this.$router.replace('/').then(() => {
            // firebaseのログインチェック時にvuexのストアもリセットする。
            commit('logout')
      ...
      alert('メールアドレスの確認が取れませんでした。登録したメールアドレスに届いたメールをご確認下さい')
    }
  },
}